...
# image 파일 경로
folder_path = r'Z:\crop_file\cropimages_0914'
if not os.path.exists(os.path.join(folder_path, 'cos_ws_2')):
    os.makedirs(os.path.join(folder_path, 'cos_ws_2'))

folder_list = os.listdir(folder_path)
for folder in folder_list :
    current_datetime = time.strftime('%Y%m%d%H%M%S', time.localtime())
    output_excel_file = os.path.join(folder_path, 'cos_ws_2', f'{folder}_{current_datetime}.xlsx')
    ...
    if len(all_files) > 1:
        ...
        with open(os.path.join(folder_path, "result_ws2.csv"), mode="a+", newline='', encoding="utf-8") as file:
            writer = csv.writer(file)

            writer.writerow([folder, len(images), count_img1, count_img2, round(count_img1 / len(images) * 100, 4), round(count_img2 / len(images) * 100, 4), sum, round(sum / len(images) * 100, 4)])